...
def plot_gaco_route(cities, route, distance, filename="TSP Solution"):
    ...
    plt.scatter(cities_np[:, 0], cities_np[:, 1],
                c='#cccccc', s=10, marker='.', label='Cities')

    path_coords = cities_np[route_np]
    plt.plot(path_coords[:, 0], path_coords[:, 1], c='blue',
             linewidth=1, alpha=0.8, label='GACO Route')

    start_city = cities_np[route_np[0]]
    plt.scatter(start_city[0], start_city[1], c='red',
                s=100, marker='*', zorder=10, label='Start/End')

    plt.title(
        f"GACO Solution for {os.path.basename(filename)}\nTotal Distance: {distance:.2f}")
...
class GACO_Large:
    def __init__(self, ttp, num_ants=50, alpha=1.0, beta=8.0, rho=0.1, Q=0.3, k_neighbors=100):
        ...
        # Size of candidate list (only look at k closest cities)
        self.k = k_neighbors
        ...
        # Indices of neighbors (excluding self)
        ...
        self.pheromone_matrix = np.full(
            (self.ttp.num_cities, self.k), self.max_tau)
    ...
    def _run_ica(self):
        ...
        sorted_items_density = sorted(
            self.ttp.items, key=lambda x: x['density'], reverse=True)
        current_weight = 0
        ...
        for area_data in sorted_areas:
            ...
            for item in unselected_type2:
                ...
                is_in_area = (area['x_min'] <= cx < area['x_max']
                              and area['y_min'] <= cy < area['y_max'])
    ...
    def _init_selection_matrix(self):
        ...
        ica_selected_items = [
            item for item in self.ttp.items if item['id'] in selected_item_ids]
    ...
    def construct_route(self):
        ...
        for step in range(1, n_cities):
            ...
            if valid_mask.any():
                ...
                if total == 0:
                    next_city = valid_neighbors[np.random.randint(
                        len(valid_neighbors))]
                else:
                    # Binary search is faster than linear scan for large K
                    r = np.random.random() * total
                    idx = np.searchsorted(cumsum, r)
                    next_city = valid_neighbors[min(
                        idx, len(valid_neighbors)-1)]
            ...
            # CASE B: No neighbors available (Rescue Mode)
                ...
                candidates = np.random.choice(
                    unvisited_indices, size=sample_size, replace=False)

                current_pos = self.ttp.cities[current_city]

                # Euclidean distance check on sample
                dists = np.sum(
                    (self.ttp.cities[candidates] - current_pos)**2, axis=1)
    ...
    def update_pheromone_trail(self, iter_best_route, iter_best_dist, global_best_route, global_best_dist):
        ...
        np.clip(self.pheromone_matrix, self.min_tau,
                self.max_tau, out=self.pheromone_matrix)
        ...
        for _ in range(max_passes):
            ...
            for i in range(n - 1):
                ...
                for v in neighbors:
                    ...
                    # Calculate gain
                    d_curr = self.ttp.get_dist(
                        u, u_next) + self.ttp.get_dist(v, v_next)
                    d_new = self.ttp.get_dist(
                        u, v) + self.ttp.get_dist(u_next, v_next)
    ...
    def full_two_opt(self, route, max_passes=5):
        ...
        if self.ttp.num_cities > 5000:
            print(
                f"Map size ({self.ttp.num_cities}) too large for Global 2-Opt.")
        ...
        while improved and count < max_passes:
            ...
            for i in range(n-1):
                A = best_route[i-1]
                # Warning: Indices slightly offset here, typically A=i, B=i+1
                B = best_route[i]
    ...
    def run(self, max_iterations=100):
        ...
        for iteration in range(max_iterations):
            ...
            opt_route, opt_dist = self.fast_two_opt(
                iter_best_route, max_passes=1)
            ...
            self.update_pheromone_trail(
                opt_route, opt_dist, best_global_route, best_global_distance)
            print(
                f"finished iteration {iteration}, best distance {best_global_distance}")
...
class MOPSO_Particle:
    ...
    def __init__(self, optimiser, capacity_fraction=1.0, random_vel=True):
    ...
    def evaluate(self):
        ...
        if current_weight > self.capacity_limit:
            ...
            base_ratios = self.optimiser.values[selected_indices] / (
                self.optimiser.weights[selected_indices] + 1e-9)
        ...
        self.current_time = self.optimiser.calculate_time(
            packing_plan, current_weight)
    ...
    def update_pbest(self):
        ...
        dominates_old = (self.current_time <= self.pbest_time) and \
                        ...
                        ((self.current_time < self.pbest_time) or (
                            self.current_profit > self.pbest_profit))
...
class MOPSO_Optimiser:
    ...
    def get_bias_for_factor(self, tightness_factor):
        ...
        city_order_map = {city_id: index for index,
                          city_id in enumerate(self.route)}
    ...
    def initialize_swarm_strategies(self, swarm_size, random_vel=True):
        """
        ...
        for i in range(swarm_size):
            ...
            p = MOPSO_Particle(self, capacity_fraction, random_vel=random_vel)
    ...
    def calculate_time(self, packing_plan, total_weight):
        ...
        for i in range(len(self.route) - 1):
            ...
            velocity = self.ttp.max_speed - \
                (current_sack_weight / self.capacity) * vel_span
            if velocity < self.ttp.min_speed:
                velocity = self.ttp.min_speed
    ...
    def prune_archive(self, max_size=200):
        ...
        # Always keep the extremes (Fastest & Richest)
        ...
        if time_range == 0 or profit_range == 0:
            return
    ...
    def run(self, swarm_size=100, iterations=100, w=0.9, c1=1.5, c2=1.5, random_vel=True):
        ...
        swarm = self.initialize_swarm_strategies(
            swarm_size, random_vel=random_vel)
...
if __name__ == "__main__":
    ...
    param_tests = [
        {"name": "w_0.8_c1_2.2_c2_2.5", "w": 0.8, "c1": 2.2, "c2": 2.5},
        {"name": "w_0.9_c1_2.2_c2_2.5", "w": 0.9, "c1": 2.2, "c2": 2.5},
        {"name": "w_1.0_c1_2.2_c2_2.5", "w": 1.0, "c1": 2.2, "c2": 2.5},
        {"name": "w_1.1_c1_2.2_c2_2.5", "w": 1.1, "c1": 2.2, "c2": 2.5},
        {"name": "w_1.5_c1_2.2_c2_2.5", "w": 1.5, "c1": 2.2, "c2": 2.5}
    ]

    all_results = {}
    FILENAME = "../resources/fnl4461-n22300.txt"
    # FILENAME = "../resources/fnl4461-n44600.txt"
    ...
    route = np.loadtxt(
        f"../../../full_route{base_name}.txt", dtype=int, delimiter=",")
