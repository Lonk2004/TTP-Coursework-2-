...
class Solution:
    ...
    def get_value(self):
        ...
        values = np.array([v['value'] for v in Solution.possible_items])
        return np.dot(self.chromosome, values)

    def get_weight(self):
        # weights = np.array([i for i, _ in Solution.bags])
        weights = np.array([w['weight'] for w in Solution.possible_items])
        return np.dot(self.chromosome, weights)

    def get_best_ratio_item(self):
        space_remaining = Solution.max_weight - self.get_weight()
        best_ratio = None
        index = None

        for i, value in enumerate(self.chromosome):
            if value ==0:
                item_weight = Solution.possible_items[i]['weight']
                # item_value = Solution.possible_items[i]['value']
                item_ratio = Solution.possible_items[i]['density']
    ...
    def calc_fitness(self):
        ...
        """
        weight_penalty = self.get_penalty()
        total_value = self.get_value()
        travel_time = self.calc_time()
        fitness = total_value - (Solution.renting_ratio * travel_time) - weight_penalty
    ...
    def repair_by_ratio(self):
        ...
        while self.get_weight() > Solution.max_weight:
    ...
    def find_worst_ratio_item(self):
        ...
        for i, value in enumerate(self.chromosome):
            if value == 1:
                ...
                ratio = Solution.possible_items[i]['density']
